...
export function HandyLine ({ bus }) {
    ...
    let h = {
      floor: ({ point }) => {
        if (handSlotType === 'input') {
          mouse.current.pos[0] = point.x
          mouse.current.pos[1] = point.y
          mouse.current.pos[2] = point.z
        }
        if (handSlotType === 'output') {
          mouse.current.pos[0] = point.x
          mouse.current.pos[1] = point.y
  ...
  useEffect(() => {
    let fromOb = scene.getObjectByName(`${'output'}${line.from}`)
    let toOb = scene.getObjectByName(`${'input'}${line.to}`)
    if (!fromOb || !toOb) {
      return
    }
    fromOb.getWorldPosition(from)
    toOb.getWorldPosition(to)
